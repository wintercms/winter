...
if(inputName!==undefined&&options.data[inputName]===undefined){options.data[inputName]=$el.val()}}var parentFormData=$form.getParentFormData()
if(options.data!==undefined&&!$.isEmptyObject(options.data)){$.extend(data,options.data)}if(useFiles){requestData=new FormData()
$.each(parentFormData,function(key){requestData.append(key,this)})
if($el.is(':file')&&inputName){$.each($el.prop('files'),function(){requestData.append(inputName,this)})
delete data[inputName]}$.each(data,function(key){if(typeof Blob!=="undefined"&&this instanceof Blob&&this.filename){requestData.append(key,this,this.filename)}else{requestData.append(key,this)}})}else{requestData=[$.param(parentFormData),$.param(data)].filter(Boolean).join('&')}var requestOptions={url:url,crossDomain:false,global:options.ajaxGlobal,context:context,headers:requestHeaders,success:function(data,textStatus,jqXHR){if(this.options.beforeUpdate.apply(this,[data,textStatus,jqXHR])===false)return
...
return this}
$.fn.getParentFormData=function(){var $form=$(this).first(),parentDataObjects=[serializeFormToObj($form)],parentFormData={};var findParentForms=function($form){if($form.length&&$form.data('request-parent')){var $parentEl=$($form.data('request-parent'));if($parentEl.length){var parentEmbeddedData={};$.each($parentEl.parents('[data-request-data]').toArray().reverse(),function extendRequest(){$.extend(parentEmbeddedData,paramToObj('data-request-data',$(this).data('request-data')));});if($parentEl.is('[data-request-data]')){$.extend(parentEmbeddedData,paramToObj('data-request-data',$parentEl.data('request-data')));}var $parentForm=$parentEl.closest('form');if($parentForm.length){parentDataObjects.push(serializeFormToObj($parentForm));findParentForms($parentForm);}}}};findParentForms($form);parentDataObjects.reverse().forEach(function(data){$.extend(parentFormData,data);});return parentFormData;}
